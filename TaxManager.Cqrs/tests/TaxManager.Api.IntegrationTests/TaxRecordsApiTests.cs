...
public class TaxRecordsApiTests(TaxManagerApiFactory factory)
{
    ...
    public async Task Create_ReturnsCreated_AndPersistsTheRecord()
    {
        ...
        var command = new AddTaxRecordCommand(municipality, TaxPeriodType.Yearly, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31), 0.25m);

        var response = await client.PostAsJsonAsync("/api/tax-records", command, JsonDefaults.Options);
    ...
    public async Task Create_ReturnsBadRequest_WhenEndDateIsBeforeStartDate()
    {
        ...
        var command = new AddTaxRecordCommand(municipality, TaxPeriodType.Yearly, new DateOnly(2024, 12, 31), new DateOnly(2024, 1, 1), 0.2m);
        var response = await client.PostAsJsonAsync("/api/tax-records", command, JsonDefaults.Options);
    ...
    public async Task Create_Allows_DifferentPeriodTypes_ToOverlapTheSameDateRange()
    {
        ...
        var yearly = new AddTaxRecordCommand(municipality, TaxPeriodType.Yearly, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31), 0.2m);
        var daily = new AddTaxRecordCommand(municipality, TaxPeriodType.Daily, new DateOnly(2024, 6, 1), new DateOnly(2024, 6, 1), 0.1m);
    ...
    public async Task Update_ChangesTheRateUsedByFutureQueries()
    {
        ...
        var created = await (await client.PostAsJsonAsync(
            "/api/tax-records",
            new AddTaxRecordCommand(municipality, TaxPeriodType.Yearly, new DateOnly(2024, 1, 1), new DateOnly(2024, 12, 31), 0.2m),
